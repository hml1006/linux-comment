...
```c
SYSCALL_DEFINE3(flistxattr, int, fd, char __user *, list, __size_t, size)
```
...
```
flistxattr (系统调用入口)
└─ path_listxattrat(AT_FDCWD, pathname, AT_EMPTY_PATH（通过 fd）, list, size)
...
```
路径解析：通过 fd 直接获取 file 对象（`AT_EMPTY_PATH`）
  │
  v
path_listxattrat()
  │
