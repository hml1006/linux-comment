...
at 系列设置扩展属性（支持 `AT_EMPTY_PATH` 和 `AT_SYMLINK_NOFOLLOW`）
...
SYSCALL_DEFINE6(setxattrat, int, dfd, const char __user *, pathname,
		...
		const struct xattr_args __user *, uargs, __size_t, usize)
```
...
```
setxattrat (系统调用入口)
└─ path_setxattrat(AT_FDCWD, pathname, at_flags（调用者指定）, name, value, size, flags)
...
```
路径解析：`dfd` + `at_flags`（由调用者指定跟随/不跟随符号链接）
  │
  v
path_setxattrat()
  │
