		...
		blk_opf_t op)
{
	...
	bio->bi_iter.bi_sector = iomap_sector(&iter->iomap, pos);  // 设置bio的起始扇区
	bio->bi_write_hint = iter->inode->i_write_hint;           // 设置写提示
	bio->bi_ioprio = dio->iocb->ki_ioprio;                     // 设置I/O优先级
	bio->bi_private = dio;                                     // 设置私有数据
	bio->bi_end_io = iomap_dio_bio_end_io;                     // 设置结束回调函数
	...
	// 根据操作类型进行相应处理
	if (dio->flags & IOMAP_DIO_WRITE)
		task_io_account_write(ret);    // 写操作，统计写入字节数
